...
class _Repository extends GroupsRepository {
  ...
  Future<GroupFinancialYearsResult> financialYears(String groupId) async =>
      GroupFinancialYearsResult(groupId: groupId, financialYears: [
        GroupFinancialYearSummary(id: 'year',
          name: 'September 2026 - September 2027 Financial Year',
          startsAt: DateTime(2026, 9, 2), endsAt: DateTime(2027, 9, 2), isActive: true),
      ]);
}
  ...
  for (final width in [320.0, 360.0, 420.0]) {
    testWidgets('report filters fit width $width', (tester) async {
      ...
      final container = ProviderContainer(overrides: [
        groupsRepositoryProvider.overrideWithValue(_Repository()),
      ]);
      addTearDown(container.dispose);
      container.read(activeGroupProvider.notifier).setGroup(const GroupAccessSummary(
        id: 'group', name: 'Group', role: 'TREASURER', status: 'ACTIVE', membersCount: 2));
      await tester.pumpWidget(UncontrolledProviderScope(container: container,
        child: MaterialApp(builder: (context, child) => MediaQuery(
          ...
          child: child!), home: const ReportFiltersScreen())));
